 ...
 */
 ...
 function validateNumbers() {
 	var numberInputs = documnet.querySelectorAll(
 		"#contactinfo input[type=number]");
 	var elementCount = numberInputs.length;
 	var numErrorDiv = documnet.getElementById("numErrorText");
 ...
 function validateForm(evt) {
 	if (evt.preventDefault) {
 		evt.preventDefault();	// prevent for from submitting
 	} else {
 		evt.returnValue = false;	// prevent form from submitting in IE8
 	}
 	formValidity = true;	// reset value for revalidatation
 	validateRequired();
 	validateNumbers();
 	if (formValidity === true) {
 		documnet.getElementsByTagName("form")[0].submit();
 	}
 }
