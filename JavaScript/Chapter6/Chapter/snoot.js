...
/* automatically check Custom message check boxif user makes
entry in customText box */
function autocheckCustom() {
	var messageBox = document.getElementById("customText");
	if (messageBox.value !== "" && messageBox.value !==
		messageBox.placeholder) {
		// if user entry is textarea, check Custom check box
...
function validatePayment() {
	...
	try {
		...
		for (var i = 0; i < elementCount; i++) {
			// verify that a month and year have neem selected
			currentElement = selectElements[i];
			if (currentElement.selectedIndex === -1) {
				currentElement.style.border = "1px solid red";
				fieldsetValidity = false;
...
function validateCreateAccount() {
	...
	try {
		...
		usernameElement.state.background = "";
		...
		if (!(usernameElement.value === "" && pass1Element.value === "" &&
			...
			throw "Please complere all fields to create an account.";
		}
	}
	catch (msg) {
		errorDiv.innerHTML = msg;
...
function validateNumbers() {
	...
	try {
		if (isNaN(ccNumElement.value) || ccNumElement.value === "") {
			...
			ccNumElement.style.display = "none";
		}
