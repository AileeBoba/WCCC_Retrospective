 ...
 */
public class ArrayBasedDeque extends Application {
  ...
  public void start(Stage stage) throws Exception {
    ...
    TextField[] qViewTextField = new TextField[10];
    ...
    outerPane.getChildren().addAll(topHBox, hBox);

    // Set the event handler on the text field
    EventHandler<ActionEvent> handler
            = new CmdTextHandler(queue, qViewTextField);
    commandEntryTextField.setOnAction(handler);

    stage.setScene(new Scene(outerPane));
    stage.setTitle("JavaFX Queue Demo");
    stage.show();
  }
}

/**
 * The handler for the command entry text field uses constructor parameters to
 * store reference to the queue and the queue view text fields.
 */
class CmdTextHandler implements EventHandler<ActionEvent> {
  ...
  /**
   * The refresh method stores the current queue entries in the corresponding
   * text fields of the queue view.
   *
   * @param The string encoding the current contents of the queue.
   */
